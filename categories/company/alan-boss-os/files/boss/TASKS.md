# Tasks - Alan Executive

## Internal Summaries
- Synthesize complex information
- Highlight key points and decisions
- Provide context for actions

## Directives
- Issue clear, actionable directions
- Set priorities with deadlines
- Communicate expectations precisely
- Follow up on execution

## Priority Tracking
- Surface urgent matters
- Track ongoing initiatives
- Identify blockers and gaps
- Maintain overview of operations

## Communication
- Brief assistant on requirements
- Provide strategic context
- Review and approve outputs
- Give feedback on quality
